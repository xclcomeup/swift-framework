...
public class GWFilterHandler extends AbstractFilterHandler implements FilterHandler {

    /**
     * GW拦截链
     */
    private List<Filter> gwFilterChains;

    public GWFilterHandler(List<Filter> gwFilterChains) {
        this.gwFilterChains = gwFilterChains;
    }

    @Override
    public boolean supports(HttpServletRequest request) {
        return UrlUtil.isGW(request.getServletPath());
    }
    ...
    public ResultStatus handle(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        // 执行拦截器
        doFilterProxy(request, response, gwFilterChains, null);
